...
Close the desktop before direct refresh; the command takes the same private-state lock and refuses concurrent service access. `download` is an alias for `refresh`:
